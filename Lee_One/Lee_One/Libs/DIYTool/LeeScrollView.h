 ...
 *  代理
 */
@protocol LeeScrollViewDelegate <NSObject>

/**
 *  该方法用来处理图片的点击，会返回图片在数组中的索引
 *  代理与block二选一即可
 *
 *  @param leeScrollView 控件本身
 *  @param index        图片索引
 */
...
@interface LeeScrollView : UIView
#pragma mark 属性
 ...
 *设置圆形图片计数的位置
 */
@property (nonatomic, assign) circlePicturePosition circlePosition;
/**
 *  占位图片，在设置图片数组之前设置
 *  不设置则为默认占位图
 */
 ...
 *  代理，用来处理图片的点击
 */
@property (nonatomic, weak) id<LeeScrollViewDelegate> delegate;
