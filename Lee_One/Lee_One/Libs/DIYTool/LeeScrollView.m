...
@interface LeeScrollView()<UIScrollViewDelegate>
//轮播的图片数组
...
@property (nonatomic, strong)UILabel *current_label;
@end
...
//创建用来缓存图片的文件夹
+ (void)initialize {
    cache = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"leeSource"];
    BOOL isDir = NO;
...
- (void)initSubView {
    ...
    if (_start) {
        ...
        [self addSubview:self.current_label];
        NSLog(@"hahahaha");
    }
}
...
- (UILabel *)current_label {
    if (!_current_label) {
        _current_label = [[UILabel alloc]init];
    }
    return _current_label;
}
- (void)setCirclePosition:(circlePicturePosition)circlePosition {
    if (circlePosition == circleBottomRight) {
        NSLog(@"+++%f",self.height);
        _total_label.frame = CGRectMake(kScreenW - 30, 170, 20, 20);
        _total_label.font = [UIFont systemFontOfSize:10];
        _current_label.frame = CGRectMake(_total_label.frame.origin.x - 20, 170, 20, 20);
        _current_label.font = [UIFont systemFontOfSize:10];
        _current_label.textAlignment = NSTextAlignmentCenter;
        _total_label.textAlignment = NSTextAlignmentCenter;
        _total_label.text = [NSString stringWithFormat:@"%ld",_imageArray.count];
        _current_label.text = [NSString stringWithFormat:@"1"];
        _total_label.layer.masksToBounds = YES;
        _total_label.layer.cornerRadius = 10;
        _current_label.layer.masksToBounds = YES;
        _current_label.layer.cornerRadius = 10;
        _total_label.backgroundColor = [UIColor redColor];
        _current_label.backgroundColor = [UIColor greenColor];
...
- (void)setImageArray:(NSArray *)imageArray{
    ...
    for (int i = 0; i < imageArray.count; i++) {
        if ([imageArray[i] isKindOfClass:[UIImage class]]) {
            ...
            else [_images addObject:[UIImage imageNamed:@"LeePlaceholder"]];
...
- (void)imageClick {
    ...
    } else if ([_delegate respondsToSelector:@selector(leeScrollView:clickImageAtIndex:)]){
        [_delegate leeScrollView:self clickImageAtIndex:self.currIndex];
    }
}
...
- (void)changeToNext {
    ...
    _current_label.text = [NSString stringWithFormat:@"%ld",self.currIndex+1];
}
